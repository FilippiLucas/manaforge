...
async function fetchFavoriteCards() {
    try {
        ...
        const favoriteCardIds = new Set(favorites.map(f => Number(f.card_id)));
        const favoriteCards = cards.filter(c => favoriteCardIds.has(c.id));

        displayCards(favoriteCards, favorites);
    } catch (err) {
...
function displayCards(cards, favorites) {
    ...
    cards.forEach(card => {
        const favoriteEntry = favorites.find(f => Number(f.card_id) === card.id);
        const $card = $(`
            <div class="p-2 card">
                <div class="relative group w-60 h-[360px] overflow-hidden rounded-lg shadow-lg transition-transform duration-300 hover:scale-[1.02] cursor-pointer">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 pointer-events-none
                        ...
                        <div class="relative z-30 flex gap-3">
                            ...
                            <button class="action remove-fav-btn bg-white/10 text-white px-3 py-2 rounded-md text-sm font-semibold hover:bg-white/20" data-fav-id="${favoriteEntry.id}">
                                ★ Remove
                            </button>
                        </div>
                    </div>
        ...
        $card.find('.add-to-deck').on('click', e => {
            e.stopPropagation();
            openDeckModalForCard(card.id);
        });

        // Remove from favorites button
        $card.find('.remove-fav-btn').on('click', async e => {
            e.stopPropagation();
            const favId = $(e.currentTarget).data('fav-id');
            try {
                await fetch(`http://localhost:3000/favorites/${favId}`, { method: 'DELETE' });
                $(e.currentTarget).closest('.card').remove(); // remove card from page
            } catch (err) {
...
$(document).ready(fetchFavoriteCards);
